...
export class World3BossCutscene {
    ...
    dialogues_en = [
        ...
        { speaker: 'Golem Watcher', text: 'Good luck, Vinn.' },
    ...
    dialogues_es = [
        ...
        { speaker: 'Vigilante Golem', text: 'Buena suerte, Vinn.' },
    ...
    update(dt: number): boolean {
        ...
        switch(this.phase) {
            case 'ALTAR_SCENE':
                ...
                break;
            case 'VINN_ARRIVES':
                this.vinnX += (300 - this.vinnX) * 0.1;
                this.vinnY += (400 - this.vinnY) * 0.1;
            ...
            case 'INK_CHASE_START':
                ...
                break;
            case 'RUN_CIRCLES':
                this.circleTimer += dt * 5;
                this.vinnX = 500 + Math.cos(this.circleTimer) * 150;
                this.vinnY = 350 + Math.sin(this.circleTimer) * 50;
                this.inkX = 500 + Math.cos(this.circleTimer - Math.PI) * 150;
                this.inkY = 350 + Math.sin(this.circleTimer - Math.PI) * 50;
    ...
    draw(ctx: CanvasRenderingContext2D) {
        ...
        // Altar specific background
        ctx.fillStyle = '#050010'; ctx.fillRect(0, 0, 1000, 500);
        ctx.fillStyle = '#100020'; ctx.fillRect(0, 390, 1000, 120);

        // Altar decorations
        ctx.fillStyle = '#2d0045';
        ctx.fillRect(450, 350, 100, 40);
        ctx.strokeStyle = '#ff00ff'; ctx.lineWidth = 2;
        ctx.strokeRect(450, 350, 100, 40);

        // Draw Blaze Priest
        this.drawBlaze(ctx, this.blazeX, this.blazeY);
        ...
        if (this.phase !== 'ALTAR_SCENE') {
            this.drawHero(ctx, this.vinnX, this.vinnY);
        }
    ...
    drawBlaze(ctx: CanvasRenderingContext2D, x: number, y: number) {
        ctx.save(); ctx.translate(x, y);
        ctx.fillStyle = '#111'; ctx.beginPath(); ctx.arc(0, -30, 20, 0, Math.PI*2); ctx.fill(); // Priest robe
        ctx.fillStyle = '#ff4500'; ctx.beginPath(); ctx.arc(0, -60, 15, 0, Math.PI*2); ctx.fill(); // Head
        ctx.strokeStyle = '#ffcc00'; ctx.lineWidth = 2; ctx.beginPath(); ctx.moveTo(-10, -70); ctx.lineTo(10, -70); ctx.stroke(); // Priest collar
        ctx.restore();
    }

    drawQueen(ctx: CanvasRenderingContext2D, x: number, y: number) {
        ctx.save(); ctx.translate(x, y);
        ctx.strokeStyle = '#ff69b4'; ctx.lineWidth = 4; ctx.beginPath(); ctx.arc(0, -50, 12, 0, Math.PI * 2); ctx.stroke();
        ctx.fillStyle = '#fff'; ctx.beginPath(); ctx.moveTo(-10, -62); ctx.lineTo(-15, -75); ctx.lineTo(0, -80); ctx.lineTo(15,-75); ctx.lineTo(10,-62); ctx.fill(); // Wedding crown
        ctx.beginPath(); ctx.moveTo(0, -38); ctx.lineTo(0, 0); ctx.stroke();
        ctx.fillStyle = '#fff'; ctx.beginPath(); ctx.moveTo(0, -30); ctx.lineTo(-20, 25); ctx.lineTo(20, 25); ctx.fill(); // Wedding dress
        ctx.restore();
    }

    drawGolem(ctx: CanvasRenderingContext2D, x: number, y: number, hasSword: boolean) {
        ctx.save(); ctx.translate(x, y);
        ctx.fillStyle = '#4a5d23'; ctx.beginPath(); ctx.rect(-30, -60, 60, 60); ctx.fill(); ctx.strokeRect(-30, -60, 60, 60);
        if (hasSword) {
            ctx.strokeStyle = '#ffaa00'; ctx.lineWidth = 4;
            ctx.beginPath(); ctx.moveTo(30, -30); ctx.lineTo(70, -70); ctx.stroke();
        }
        ctx.restore();
    }
    ...
    drawInk(ctx: CanvasRenderingContext2D, x: number, y: number) {
        ctx.save(); ctx.translate(x, y);
        ctx.fillStyle = '#000'; ctx.shadowBlur = 10; ctx.shadowColor = '#ff00ff';
        ctx.beginPath(); ctx.arc(0, -50, 40, 0, Math.PI*2); ctx.fill();
        ctx.fillStyle = '#ff00ff'; ctx.beginPath(); ctx.arc(-15, -60, 5, 0, Math.PI*2); ctx.arc(15, -60, 5, 0, Math.PI*2); ctx.fill();
        ctx.restore();
    }

    drawHero(ctx: CanvasRenderingContext2D, x: number, y: number) {
        ctx.save(); ctx.translate(x, y);
